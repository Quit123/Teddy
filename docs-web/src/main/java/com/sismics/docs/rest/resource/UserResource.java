...
public class UserResource extends BaseResource {
    ...
    public Response registerRequest(
        ...
        // 2. 检查用户名是否已存在
        UserDao userDao = new UserDao();
        if (userDao.getActiveByUsername(username) != null /*|| userDao.getByUsername(username) != null*/) {
            // 若已存在同名用户（不论已激活或待批准），抛出错误
            throw new ClientException("AlreadyExistingUsername", "用户名已被占用");
        }

        // 3. 创建待批准用户对象
        User user = new User();
        user.setRoleId(Constants.DEFAULT_USER_ROLE);  // 默认角色：普通用户
        user.setUsername(username);
        user.setPassword(password);
        user.setEmail(email);
        // 设置存储配额，可使用系统默认值或配置值
        user.setStorageQuota(50000000l);
        user.setOnboarding(true);

        user.setStorageCurrent(0L);  // 初始占用空间
        user.setPrivateKey(EncryptionUtil.generatePrivateKey()); // 系统要求字段
        // 标记为未激活/待批准：
        user.setDisableDate(new Date());  // 将账户设为已禁用状态表示待审批

        try {
            // 4. 保存用户（创建禁用状态的账户）
            userDao.create(user, null);  // 创建用户记录，创建人参数可传 null 或特定标识（如系统）
        } catch (Exception e) {
            if ("AlreadyExistingUsername".equals(e.getMessage())) {
                throw new ClientException("AlreadyExistingUsername", "用户名已被占用", e);
            } else {
                throw new ServerException("UnknownError", "服务器错误", e);
            }
        }
        // 5. 返回结果
    ...
    public Response admitUser(@PathParam("username") String username) {
        ...
        UserDao userDao = new UserDao();
        // 查找用户，包括禁用的待批准用户
        User user = userDao.getAnyByUsername(username);
//        if (user == null) {
//            user = userDao.getByUsername(username);  // 尝试获取即使被禁用的用户
//        }
        if (user == null) {
        ...
        if (user.getDisableDate() != null) {
            user.setDisableDate(null); // ✅ 去掉禁用标记
